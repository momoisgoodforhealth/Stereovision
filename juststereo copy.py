...
for i in tqdm(range(120,180)):
  ...
  imgR_gray = cv2.imread(pathR+"chessboard-R%d.png"%i,0)
...
img_ptsL = np.array(img_ptsL, dtype=np.float32)

# Calibrating left camera
retL, mtxL, distL, rvecsL, tvecsL = cv2.calibrateCamera(obj_pts,img_ptsL,imgL_gray.shape[::-1],None,None)
projectedPoints, _ = cv2.projectPoints(obj_pts, rvecsL[0], tvecsL[0], mtxL, distL)

# Compute reprojection error for each image
reprojectionErrors = []
for i in range(len(img_ptsL)):
    error = np.linalg.norm(projectedPoints[i].flatten() - img_ptsL[i])
    reprojectionErrors.append(error)

# Calculate statistics on reprojection errors
maxReprojectionError = np.max(reprojectionErrors)
avgReprojectionError = np.mean(reprojectionErrors)
